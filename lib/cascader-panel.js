  ...
  methods: {
    ...
    lazyLoad: function lazyLoad(node, onFullfiled) {
      ...
      var resolve = function resolve(dataList) {
        ...
        if (Array.isArray(_this4.checkedValue)) {
          var nodeValue = _this4.checkedValue[_this4.loadCount++];
          var valueKey = _this4.config.value;
          var leafKey = _this4.config.leaf;

          if (Array.isArray(dataList) && dataList.filter(function (item) {
            return item[valueKey] === nodeValue;
          }).length > 0) {
            var checkedNode = _this4.store.getNodeByValue(nodeValue);

            if (!checkedNode.data[leafKey]) {
              _this4.lazyLoad(checkedNode, function () {
                _this4.handleExpand(checkedNode);
              });
            }

            if (_this4.loadCount === _this4.checkedValue.length) {
              _this4.$parent.computePresentText();
            }
          }
        }
